...
if ( $our_team->have_posts() ) {
	...
	while ( $our_team->have_posts() ): $our_team->the_post();
		...
		$html .= '<div class="our-team-item col-sm-' . $columns . '"><div class="item">';
		$html .= '<div class="our-team-image"> ' . thim_get_feature_image( get_post_thumbnail_id(), 'full', apply_filters( 'thim_member_thumbnail_width', 270 ), apply_filters( 'thim_member_thumbnail_height', 275 ) ) . '</div>';
		...
		if ( $link_face <> '' ) {
			$html .= '<a class="facebook" href="' . $link_face . '"><i class="fa fa-facebook"></i></a>';
		}
		if ( $link_twitter <> '' ) {
			$html .= '<a class="twitter" href="' . $link_twitter . '"><i class="fa fa-twitter"></i></a>';
		}
		if ( $dribbble_url <> '' ) {
			$html .= '<a class="dribbble" href="' . $dribbble_url . '"><i class="fa fa-dribbble"></i></a>';
		}
		if ( $skype_url <> '' ) {
			$html .= '<a class="skype" href="' . $skype_url . '"><i class="fa fa-skype"></i></a>';
		}
		if ( $linkedin_url <> '' ) {
			$html .= '<a class="linkedin" href="' . $linkedin_url . '"><i class="fa fa-linkedin"></i></a>';
		}
		$html .= '</div>';
		$html .= '<div class="thim-background-border"></div>';
		$html .= '<div class="description">' . thim_excerpt( 15 ) . '</div>';
		$html .= '</div></div></div>';
	endwhile;
	$html .= '</div></div>';
}
